...
# Overall, the function will take O(N * logN + N^2) , which is asymptotically
# equivalent to O(N^2).
#
# Space complexity#
...
#               return target_sum # return sum of all the numbers
#
#         # the second part following the 'if' is to handle the smallest sum when
#         # we have more than one solution
#           if abs(target_diff) < abs(smallest_difference) or (abs(target_diff) == abs(smallest_difference) and target_diff > smallest_difference):
#             smallest_differece = target_diff
#
#           if target_diff > 0:
